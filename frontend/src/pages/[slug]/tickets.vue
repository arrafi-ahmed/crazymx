<script setup>
  ...
  async function proceedToForm () {
    ...
    try {
      ...
      if (!storedData) {
        store.commit('addSnackbar', { text: 'Please complete the registration form first', color: 'error' })
        return
      }
      ...
      store.commit('addSnackbar', { text: 'Failed to proceed to attendee forms. Please try again.', color: 'error' })
    } finally {
