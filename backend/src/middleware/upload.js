...
function createUpload(prefix) {
    return (req, res, next) => {
        ...
        busboy.on("file", (name, file, info) => {
            ...
            const newName =
                prefix + "-" + Date.now() + "-" + Math.round(Math.random() * 1e5) + ext;
...
const uploadEventBanner = createUpload("eventBanner");
const uploadClubLogo = createUpload("clubLogo");
const uploadTmp = createUpload("tmp");

module.exports = {uploadUser, uploadEventBanner, uploadClubLogo, uploadTmp};
